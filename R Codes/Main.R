...
spain.mainland <- raster::shapefile("maps/Extras/Spain_mainland.shp")
extent(spain.mainland)
...
plot(raster(g1kx["pred"]), col=brewer.pal(n = 9, name = "GnBu"),
     main="Prediction EML", axes=FALSE, box=FALSE, ext= extent(spain.mainland))
...
plot(raster(g1kx["model.error"]), col=rev(bpy.colors()),
     main="Permissible Prediction errors", axes=FALSE, box=FALSE, ext= extent(spain.mainland))
#points(occ.datapts[occ.datapts$year == out.year, ], pch="+", cex=.8)
}
...
plot(trendmap[["SlopeSEG1"]], 
     ...
     zlim=c(-1.5,1.5), main="Slope SEG1", ext= extent(spain.mainland))
...
plot(raster(spain.trend[1]), 
     col=rev(SAGA_pal[["SG_COLORS_GREEN_GREY_RED"]]), main="Slope SEG1", ext= extent(spain.mainland))
